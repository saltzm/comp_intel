...
class ForestPlanningFitnessFunction (adjacencyFile: String, 
                                     volumeFile: String,
                                     numberOfStands: Int,
                                     numberOfTimePeriods: Int,
                                     target: Int) extends FitnessFunction { 
  //adjacency lists
  ...
  override def getFitness[T](g: Genotype[T]): Double = {
    calculateAndSaveFitness(g)
  }

  def calculateAndSaveFitness[T](g: Genotype[T]): Double = {
  ...
  def getPeriodHarvests[T](g: Genotype[T]): Array[Double] = {
    val genes = g.genes.map( x => x.asInstanceOf[Int] )
    var hArray = Array.ofDim[Double](numberOfTimePeriods)
