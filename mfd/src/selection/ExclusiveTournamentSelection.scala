...
class ExclusiveTournamentSelectionEngine(k: Int, p: Double, elit: Boolean) extends SelectionEngine {
  ...
  def select(population: BitBasedPopulation) {
    ...
    var popSet = population.population.toSet
    var notPicked = popSet.iterator
    while (counter < population.size) {
      ...
      for (i <- 0 until brackSize) {
        if (notPicked.isEmpty){ notPicked = popSet.iterator}
        var competitor = notPicked.next()
